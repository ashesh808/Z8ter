...
export default function initAbout(): void {
  ...
  const themeBtn = root.querySelector("#about-theme") as HTMLButtonElement | null;
  ...
  const copyBtn = root.querySelector("#about-copy") as HTMLButtonElement | null;
  ...
  const net = root.querySelector("#about-net") as HTMLElement | null;
  const updateNet = () => {
    if (!net) return;
  ...
  const clock = root.querySelector("#about-clock") as HTMLElement | null;
  ...
  (root as any)._cleanup = () => clearInterval(clockId);

  // PERSISTED PREFERENCE (localStorage)
  const pref = root.querySelector("#about-island-pref") as HTMLInputElement | null;
  try { if (pref) pref.checked = localStorage.getItem(ISLAND_PREF_KEY) === "1"; } catch {}
  pref?.addEventListener("change", () => {
    try { localStorage.setItem(ISLAND_PREF_KEY, pref.checked ? "1" : "0"); } catch {}
  });

  // LATENCY BENCHMARK + PRETTY JSON (client-side formatting)
  const pingBtn = root.querySelector("#about-ping") as HTMLButtonElement | null;
  const pingOut = root.querySelector("#about-ping-out") as HTMLElement | null;
  pingBtn?.addEventListener("click", async () => {
    if (!pingOut || !pingBtn) return;
